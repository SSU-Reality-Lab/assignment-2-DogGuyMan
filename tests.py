...
1. SSD는 비슷한 디스크립터가 여럿 존재하거나, 아예 대응점이 없는경우에도
오직 맹목적인 거리 우선으로 매칭을 수행하기 때문에 Outlier가 생긴다. 
그리고 이것은 추후 (이미지 정렬, 호모그래피 추정) 에 있어 오차 제곱합의 결과는 Outlier에 매우 민감하므로 잘못된 결과를 낼 가능성이 있다

2. 하지만 Ratio Test는 후보 1, 2(일명 최근접 후보들)를 나눠 
거리 비(Ratio)를 통해 서로의 거리 유사도 검사를 진행한다.
