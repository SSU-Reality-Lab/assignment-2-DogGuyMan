...
1. matcher_ssd 클래스의 matchFeatures 메서드는 비슷한 디스크립터가 여럿 존재하거나, 아예 대응점이 없는 경우에도
오직 맹목적인 거리 우선으로 매칭을 수행하기 때문에 가장 작은 SSD를 가지는 것을 매칭하는데
이것은 Outlier와 매칭되거나, 매우 모호한 특징점끼리 잘못 매칭될 여지를 만든다.
...
이 아예 0이랑 구별할 수 없을 정도로 일치한다면 바로 후보 1을 매칭하지만, 
